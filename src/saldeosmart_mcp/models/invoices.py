...
from ..http.xml import parse_int_list
...
class InvoiceIdGroups(BaseModel):
    ...
    def from_xml(cls, root: ET.Element) -> InvoiceIdGroups:
        return cls(
            invoices=parse_int_list(root, "INVOICES", "INVOICE_ID"),
            corrective_invoices=parse_int_list(
                root, "CORRECTIVE_INVOICES", "CORRECTIVE_INVOICE_ID"
            ),
            pre_invoices=parse_int_list(root, "PRE_INVOICES", "PRE_INVOICE_ID"),
            corrective_pre_invoices=parse_int_list(
                root, "CORRECTIVE_PRE_INVOICES", "CORRECTIVE_PRE_INVOICE_ID"
            ),
        )
...
class InvoiceAddInput(BaseModel):
    ...
    currency_date: IsoDate | None = None
